...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\cn\\Desktop\\ldmm_fmri-main\\DecisionLab_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    key_resp_17 = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    text_21 = visual.TextStim(win=win, name='text_21',
        ...
        languageStyle='LTR',
        depth=-2.0);
    key_resp_16 = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    text_22 = visual.TextStim(win=win, name='text_22',
        ...
        languageStyle='LTR',
        depth=-2.0);
    key_resp_18 = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    text_20 = visual.TextStim(win=win, name='text_20',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_19 = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    # Decision ITI defaults
    DECISION_ITI12_VALUES = [1, 2, 3]  # ITI_1 and ITI_2
    DECISION_ITI3_VALUES = [2, 3, 4]   # ITI_3 (crosshair after choice)
    
    # Optional FAST_MODE (set in PsychoPy dialog as expInfo field "fast_mode")
    FAST_MODE = False
    try:
        if 'thisExp' in locals() and hasattr(thisExp, 'info') and 'fast_mode' in thisExp.info:
            FAST_MODE = bool(int(thisExp.info['fast_mode'])) if str(thisExp.info['fast_mode']).strip().isdigit() else bool(thisExp.info['fast_mode'])
        elif 'expInfo' in locals() and 'fast_mode' in expInfo:
            FAST_MODE = bool(int(expInfo['fast_mode'])) if str(expInfo['fast_mode']).strip().isdigit() else bool(expInfo['fast_mode'])
    except Exception:
        FAST_MODE = False
    
    if FAST_MODE:
        # Make everything code-controlled extremely short for quick testing
        TRIAL_TIMEOUT_SECONDS = 0.4
        CROSSHAIR_RANDOM_VALUES = [0.1]
        TIMEOUT_SCREEN_DURATION = 0.1
        ACTUAL_POINTS_DURATION = 0.1
        DECISION_TIMEOUT_DURATION = 0.4
        DECISION_ITI12_VALUES = [0.1]
        DECISION_ITI3_VALUES = [0.1]
        print("FAST_MODE enabled: using very short timings for rapid testing")
    
    # Store timing config in globals so other routines don't fall back to defaults
    globals()['FAST_MODE'] = FAST_MODE
    globals()['TRIAL_TIMEOUT_SECONDS'] = TRIAL_TIMEOUT_SECONDS
    globals()['CROSSHAIR_RANDOM_VALUES'] = CROSSHAIR_RANDOM_VALUES
    globals()['ACTUAL_POINTS_DURATION'] = ACTUAL_POINTS_DURATION
    ...
    globals()['TIMEOUT_SCREEN_DURATION'] = TIMEOUT_SCREEN_DURATION
    globals()['DECISION_ITI12_VALUES'] = DECISION_ITI12_VALUES
    globals()['DECISION_ITI3_VALUES'] = DECISION_ITI3_VALUES
    ...
    if DAY == 2:
        ...
        if not has_complete_data:
            ...
            print(f"  Looking in: data/sub-1/")
            print(f"  NOTE: Day 1 must have completed all trials to use Day 2 counterbalancing")
            print(f"  Falling back to random assignment (Day 1 behavior)")
            # Fall back to Day 1 behavior
            ...
            # Counterbalance Day 2:
            # If Day 1 was gain first, randomly choose gain or loss first for Day 2
            # If Day 1 was loss first, randomly choose gain or loss first for Day 2
            day2_first_block = random.choice(["gain", "loss"])
            print(f"  Day 2 first block (random counterbalance): {day2_first_block}")
            
            # Switch fruit assignments: Day 1 gains become Day 2 losses, Day 1 losses become Day 2 gains
            # Also shift values: 20→50, 50→80, 80→20 (and -20→-50, -50→-80, -80→-20)
            ...
            # Value mapping function: 20→50, 50→80, 80→20 (and same for negatives)
            def shift_value(val):
                """Map values: 20→50, 50→80, 80→20 (and -20→-50, -50→-80, -80→-20)"""
                if val == 20:
                    return 50
                elif val == 50:
                    return 80
            ...
            # Day 1 losses become Day 2 gains (with shifted values)
            day2_gain_vals = [shift_value(val) for val in day1_loss_vals]
            # Day 1 gains become Day 2 losses (with shifted values)
            day2_loss_vals = [shift_value(val) for val in day1_gain_vals]
            
            # Assign fruits: Day 1 losses become Day 2 gains, Day 1 gains become Day 2 losses
            gain_fruits = day1_loss_fruits.copy()
            loss_fruits = day1_gain_fruits.copy()
            
            # Create baseline lists with shifted values (matching fruit order)
            gain_baselines = day2_gain_vals
            loss_baselines = day2_loss_vals
            
            print(f"Day 2 assignments (switched and shifted):")
    ...
    if DAY == 2:
        ...
        if day2_first_block:
            ...
            print(f"Day 2 block order: {first_block} first, then {second_block} (counterbalanced from Day 1)")
        else:
            # Fallback if day2_first_block wasn't set
            block_order = ["gain", "loss"]
            random.shuffle(block_order)
            first_block = block_order[0]
            second_block = block_order[1]
            print(f"Day 2 block order: {first_block} first, then {second_block} (fallback random)")
    else:
    ...
    scanner_block1_was_shown = False  # Set True when scannerSync runs (not skipped); only then does End Routine reset clock
    globals()['scanner_block1_was_shown'] = scanner_block1_was_shown
    ...
    # Rep template (matches your MATLAB outline):
    ...
    for fruit1, fruit2 in decision_pairs:
        ...
        # rep 1: left=fruit1 right=fruit2; first=fruit1 on left
        ...
        # rep 2: left=fruit1 right=fruit2; first=fruit2 on right
        decision_block.append({
            "leftFruit": fruit1,
            "rightFruit": fruit2,
            "firstFruit": fruit2,
        ...
        # rep 3: left=fruit2 right=fruit1; first=fruit1 on right (so fruit1 ends on right)
        decision_block.append({
            "leftFruit": fruit2,
            "rightFruit": fruit1,
            "firstFruit": fruit1,
        ...
        # rep 4: left=fruit2 right=fruit1; first=fruit2 on left (so fruit2 ends on left)
        decision_block.append({
            "leftFruit": fruit2,
            "rightFruit": fruit1,
            "firstFruit": fruit2,
    ...
    # Mixed gain/loss: either 1 gain + 2 loss or 1 loss + 2 gain (randomly chosen)
    # Avoid 3 gains or 3 losses
    practice_block = []
    # Static practice images (always the same)
    practice_fruit_names = ["dog.png", "cat.png", "rabbit.png"]
    
    # Gain and loss value sets (for each fruit: dog, cat, rabbit)
    gain_values = [30, 50, 10]
    gain_baselines = [50, 50, 50]
    loss_values = [-30, -50, -10]
    loss_baselines = [-50, -50, -50]
    
    # Randomly choose mix: either 1 gain + 2 loss or 1 loss + 2 gain
    mix_type = random.choice(["1g2l", "1l2g"])  # 1 gain 2 loss, or 1 loss 2 gain
    if mix_type == "1g2l":
        trial_types = ["gain", "loss", "loss"]
    else:
        trial_types = ["loss", "gain", "gain"]
    random.shuffle(trial_types)  # Randomize which position (dog/cat/rabbit) gets each type
    
    for i, image_name in enumerate(practice_fruit_names):
        if trial_types[i] == "gain":
            practice_value = gain_values[i]
            baseline = gain_baselines[i]
            practice_block_type_i = "gain"
        else:
            practice_value = loss_values[i]
            baseline = loss_baselines[i]
            practice_block_type_i = "loss"
        
        practice_block.append({
            "block": practice_block_type_i,
            "fruit": image_dir + image_name,
            "baseline": baseline,
            "true_value": practice_value
        })
        print(f"Practice trial {i+1} (Block 1): {image_name} with {practice_block_type_i} value {practice_value:+d} (baseline: {baseline:+d})")
    ...
    text_block_transition = visual.TextStim(win=win, name='text_block_transition',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_transition = keyboard.Keyboard(deviceName='defaultKeyboard')
    
    # --- Initialize components for Routine "taskStartSoonBlock2" ---
    key_resp_task_start_block2 = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    Welcome = data.Routine(
        name='Welcome',
        components=[text, apple, banana, grape, kiwi, pear, strawberry, key_resp_17],
    )
    ...
    while continueRoutine:
        ...
        if key_resp_17.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_17.getKeys(keyList=['c', 'C'], ignoreKeys=["escape"], waitRelease=False)
    ...
    thisExp.currentRoutine = TaskStructure
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if TaskStructure.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in TaskStructure.components:
    ...
    thisExp.currentRoutine = yourGoal
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if yourGoal.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in yourGoal.components:
    ...
    thisExp.currentRoutine = keyInstructions
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if keyInstructions.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in keyInstructions.components:
    ...
    thisExp.currentRoutine = practiceIntro
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if practiceIntro.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in practiceIntro.components:
    ...
    for thisPracticeTrial in practiceTrials:
        ...
        thisExp.currentRoutine = crosshair5
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            # has the Routine been forcibly ended?
            if crosshair5.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshair5.components:
        ...
        thisExp.currentRoutine = actualPoints1
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # has the Routine been forcibly ended?
            if actualPoints1.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in actualPoints1.components:
        ...
        thisExp.currentRoutine = crosshair3
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            # has the Routine been forcibly ended?
            if crosshair3.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshair3.components:
    ...
    thisExp.currentRoutine = taskStartSoon
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if taskStartSoon.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in taskStartSoon.components:
    ...
    if block1_scanner_sync_complete or block2_setup_complete or learning_block_index >= 0:
        ...
        continueRoutine = False
        globals()['scanner_block1_was_shown'] = False
    else:
        globals()['scanner_block1_was_shown'] = True  # Only reset clock in End Routine when we actually ran
        # Clear any previous key presses
        try:
            event.clearEvents()
        except:
    ...
    thisExp.currentRoutine = scannerSync
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if scannerSync.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in scannerSync.components:
    ...
    # Only reset clock and run cleanup when scannerSync actually ran (user pressed 5)
    # When skipped (block1_scanner_sync_complete already True), End Routine still runs - do NOT reset clock
    scanner_block1_was_shown = globals().get('scanner_block1_was_shown', False)
    
    if scanner_block1_was_shown:
        ...
        globals()['scanner_block1_was_shown'] = False  # reset for safety
        print("Block 1 scanner sync complete - will not show again")
    
        # === Global clock reset on scanner trigger (Block 1) ===
        import psychopy.core as psychopy_core
        if 'scanner_clock' not in globals() or globals()['scanner_clock'] is None:
            globals()['scanner_clock'] = psychopy_core.Clock()
        else:
            globals()['scanner_clock'].reset()
        globals()['scanner_sync_block'] = 1
        print(f"[Block 1] Scanner clock reset at trigger '5'")
    thisExp.nextEntry()
    ...
    thisExp.currentRoutine = scannerWait
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # has the Routine been forcibly ended?
        if scannerWait.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in scannerWait.components:
    ...
    thisExp.currentRoutine = startTask
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        # has the Routine been forcibly ended?
        if startTask.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in startTask.components:
    ...
    for thisLearningLoop in learningLoop:
        ...
        while continueRoutine:
            ...
            for key in keys:
                ...
                # === LOG FIRST 1 OR 3 PRESS (relative to scanner sync) ===
                scanner_clock = globals().get('scanner_clock', None)
                relative_time = scanner_clock.getTime() if scanner_clock else None
            
                if key in ('1', 'num_1', '3', 'num_3') and not globals().get('first_response_logged', False):
                    globals()['first_response_time'] = relative_time
                    globals()['first_response_logged'] = True
                    print(f"First slider key ({key}) at {relative_time:.4f}s since scanner sync")
            
                # === LOG KEY 2 SUBMIT (relative to scanner sync) ===
                if key in ('2', 'num_2'):
                    globals()['submit_time'] = relative_time
                    print(f"Submit (key 2) at {relative_time:.4f}s since scanner sync")
        ...
        # === NEW: log timestamps to PsychoPy's own data file ===
        thisExp.addData("fruit_onset_time", globals().get('fruit_onset_time', None))
        thisExp.addData("first_response_time", globals().get('first_response_time', None))
        thisExp.addData("submit_time", globals().get('submit_time', None))
        ...
        if responded_in_time and actual_response_time <= TRIAL_TIMEOUT_SECONDS:
            ...
            if FAST_MODE:
                # In FAST_MODE, don't pad to full trial length; keep everything minimal
                crosshair1_duration = 0.0
                actualPoints_duration = 0.0
                rand2 = random.choice(CROSSHAIR_RANDOM_VALUES)
                crosshair2_duration = 0.0
            ...
            globals()['crosshair1_rand'] = rand1
            globals()['crosshair2_rand'] = rand2
        ...
        try:
            ...
            if data_logger:
                ...
                data_logger.log_learning_trial(
                    ...
                    fruit_onset_time=globals().get('fruit_onset_time', None),
                    first_response_time=globals().get('first_response_time', None),
                    submit_time=globals().get('submit_time', None),
                )
        except Exception as e:
        ...
        print(f"DEBUG END ROUTINE: learning_block_index={learning_block_index}, trials_per_block={trials_per_block}, total_expected={total_expected}")
        print(f"DEBUG FLAGS: show_block_transition={globals().get('show_block_transition', False)}, ready_for_block2_setup={globals().get('ready_for_block2_setup', False)}, block2_setup_complete={globals().get('block2_setup_complete', False)}")
        print(f"DEBUG CONDITION: learning_block_index == trials_per_block - 1 → {learning_block_index} == {trials_per_block - 1} → {learning_block_index == trials_per_block - 1}")
        ...
        # --- Run Routine "timeout" ---
        thisExp.currentRoutine = timeout
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if timeout.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in timeout.components:
        ...
        thisExp.currentRoutine = crosshair1
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if crosshair1.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshair1.components:
        ...
        thisExp.currentRoutine = actualPoints
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if actualPoints.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in actualPoints.components:
        ...
        thisExp.currentRoutine = crosshair2
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if crosshair2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshair2.components:
        ...
        if not show_block_transition or ready_for_block2_setup:
            ...
            globals()['block_transition_was_shown'] = False
            continueRoutine = False
        else:
            # Mark that this routine is actually running
            globals()['block_transition_was_shown'] = True
        ...
        thisExp.currentRoutine = blockTransition
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if blockTransition.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in blockTransition.components:
        ...
        # Only execute if the routine actually ran (not skipped)
        show_block_transition_was_shown = globals().get('block_transition_was_shown', False)
        
        if show_block_transition_was_shown:
            if 'text_block_transition' in locals():
            ...
            globals()['show_block_transition'] = False
            globals()['ready_for_block2_setup'] = True
            globals()['block_transition_was_shown'] = False  # reset for safety
            ...
            print("Block transition end routine: routine was skipped, doing nothing")
        ...
        if not ready_for_block2_setup or block2_setup_complete:
            ...
            globals()['task_start_block2_was_shown'] = False
            continueRoutine = False
        
        else:
            # Mark that this routine is actually running
            globals()['task_start_block2_was_shown'] = True
        ...
        thisExp.currentRoutine = taskStartSoonBlock2
        ...
        while continueRoutine:
            ...
            if block2_setup_complete or not ready_for_scanner_sync_block2:    # Block transition hasn't completed yet OR block 2 setup already done OR still in Block 1 OR already in Block 2 - skip
                continueRoutine = False
            else:
            ...
            # has the Routine been forcibly ended?
            if taskStartSoonBlock2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in taskStartSoonBlock2.components:
        ...
        task_start_block2_was_shown = globals().get('task_start_block2_was_shown', False)
        
        if task_start_block2_was_shown:
            if 'text_task_start_soon_block2' in locals():
            ...
            try:
                ...
                pass
            globals()['ready_for_scanner_sync_block2'] = True
            globals()['task_start_block2_was_shown'] = False  # reset for safety
            print("Task start soon Block 2 ended: Set ready_for_scanner_sync_block2 = True")
        else:
            print("taskStartSoonBlock2 end routine: routine was skipped, doing nothing")
        # check responses
        ...
        if block2_setup_complete or not ready_for_scanner_sync_block2:
            if block2_setup_complete:
            ...
            # Mark that this routine is actually running
            globals()['scanner_block2_was_shown'] = True
        ...
        thisExp.currentRoutine = scannerSyncBlock2
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if scannerSyncBlock2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in scannerSyncBlock2.components:
        ...
        scanner_block2_was_shown = globals().get('scanner_block2_was_shown', False)
        
        if scanner_block2_was_shown:
            if 'text_scanner_wait_block2' in locals():
            ...
            try:
                ...
                pass
            # NOTE: Do NOT set block2_setup_complete here - set in crosshair2Block2 End Routine
            # so crosshairBlock2, scannerWaitBlock2, crosshair2Block2 run (~4.5s before first fruit)
            globals()['scanner_block2_was_shown'] = False  # reset for safety
            print("Block 2 scanner sync complete")
            # === Global clock reset on scanner trigger (Block 2) ===
            import psychopy.core as psychopy_core
            scanner_clock = globals().get('scanner_clock', None)
            if scanner_clock is not None:
                scanner_clock.reset()
                ...
                globals()['scanner_clock'] = psychopy_core.Clock()
            globals()['scanner_sync_block'] = 2
            print(f"[Block 2] Scanner clock reset at trigger '5'")
        else:
            print("scannerSyncBlock2 end routine: routine was skipped, doing nothing")
        # check responses
        ...
        thisExp.currentRoutine = crosshairBlock2
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # has the Routine been forcibly ended?
            if crosshairBlock2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshairBlock2.components:
        ...
        # Hide crosshair Block 2 text when routine ends
        if 'text_crosshair_block2' in locals():
            text_crosshair_block2.setAutoDraw(False)
            text_crosshair_block2.opacity = 0
            print("Crosshair Block 2 text hidden")
        ...
        # Do NOT set block2_setup_complete here - wait until after scannerWaitBlock2 and final crosshair
        ...
        thisExp.currentRoutine = scannerWaitBlock2
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # has the Routine been forcibly ended?
            if scannerWaitBlock2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in scannerWaitBlock2.components:
        ...
        # Hide scanner wait Block 2 image when routine ends
        if 'image_scanner_wait_block2' in locals():
            image_scanner_wait_block2.setAutoDraw(False)
            image_scanner_wait_block2.opacity = 0
            print("Scanner wait Block 2 image hidden")
        ...
        thisExp.currentRoutine = crosshair2Block2
        ...
        while continueRoutine and routineTimer.getTime() < 0.5:
            ...
            if text_crosshair2_block2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_crosshair2_block2.tStartRefresh + .5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # has the Routine been forcibly ended?
            if crosshair2Block2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshair2Block2.components:
    ...
    thisExp.currentRoutine = decisionPhase
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if decisionPhase.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in decisionPhase.components:
    ...
    thisExp.currentRoutine = decisionInstructions
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if decisionInstructions.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in decisionInstructions.components:
    ...
    thisExp.currentRoutine = whichFruit
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if whichFruit.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in whichFruit.components:
    ...
    thisExp.currentRoutine = practiceIntro2
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if practiceIntro2.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in practiceIntro2.components:
    ...
    for thisPracticeDecisionLoop in practiceDecisionLoop:
        ...
        thisExp.currentRoutine = practiceDecisionSimulus1
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if practiceDecisionSimulus1.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in practiceDecisionSimulus1.components:
        ...
        crosshairPracticeDecision1.maxDuration = 0
        # keep track of which components have finished
        ...
        thisExp.currentRoutine = crosshairPracticeDecision1
        ...
        while continueRoutine:
            ...
            # update/draw components on each frame
            # is it time to end the Routine? (based on local clock)
            if tThisFlip > crosshairPracticeDecision1.maxDuration-frameTolerance:
                crosshairPracticeDecision1.maxDurationReached = True
                continueRoutine = False
            ...
            # has the Routine been forcibly ended?
            if crosshairPracticeDecision1.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshairPracticeDecision1.components:
        ...
        thisExp.currentRoutine = practiceDecisionSimulus2
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if practiceDecisionSimulus2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in practiceDecisionSimulus2.components:
        ...
        crosshairPracticeDecision2.maxDuration = 0
        # keep track of which components have finished
        ...
        thisExp.currentRoutine = crosshairPracticeDecision2
        ...
        while continueRoutine:
            ...
            # update/draw components on each frame
            # is it time to end the Routine? (based on local clock)
            if tThisFlip > crosshairPracticeDecision2.maxDuration-frameTolerance:
                crosshairPracticeDecision2.maxDurationReached = True
                continueRoutine = False
            ...
            # has the Routine been forcibly ended?
            if crosshairPracticeDecision2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshairPracticeDecision2.components:
        ...
        thisExp.currentRoutine = practiceDecisionMaking
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if practiceDecisionMaking.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in practiceDecisionMaking.components:
        ...
        crosshairPracticeDecision3.maxDuration = 0
        # keep track of which components have finished
        ...
        thisExp.currentRoutine = crosshairPracticeDecision3
        ...
        while continueRoutine:
            ...
            # update/draw components on each frame
            # is it time to end the Routine? (based on local clock)
            if tThisFlip > crosshairPracticeDecision3.maxDuration-frameTolerance:
                crosshairPracticeDecision3.maxDurationReached = True
                continueRoutine = False
            ...
            # has the Routine been forcibly ended?
            if crosshairPracticeDecision3.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in crosshairPracticeDecision3.components:
    ...
    thisExp.currentRoutine = taskStartSoonBlock3
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if taskStartSoonBlock3.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in taskStartSoonBlock3.components:
    ...
    thisExp.currentRoutine = scannerSyncBlock3
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if scannerSyncBlock3.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in scannerSyncBlock3.components:
    ...
    # Hide scanner wait text (Block 3)
    if 'text_scanner_wait_block3' in locals():
        text_scanner_wait_block3.setAutoDraw(False)
        text_scanner_wait_block3.opacity = 0
    try:
        event.clearEvents()
    except:
        pass
    
    # === Global clock reset on scanner trigger (Block 3) ===
    import psychopy.core as psychopy_core
    scanner_clock = globals().get('scanner_clock', None)
    if scanner_clock is not None:
        scanner_clock.reset()
        ...
        globals()['scanner_clock'] = psychopy_core.Clock()
    globals()['scanner_sync_block'] = 3
    print(f"[Block 3] Scanner clock reset at trigger '5'")
    
    # Reset decision index for this block
    globals()['decision_index'] = 0
    print("Block 3 scanner sync complete - decision phase starting")
    thisExp.nextEntry()
    ...
    thisExp.currentRoutine = crosshair3Block3
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # has the Routine been forcibly ended?
        if crosshair3Block3.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in crosshair3Block3.components:
    ...
    thisExp.currentRoutine = scannerWaitBlock3
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # has the Routine been forcibly ended?
        if scannerWaitBlock3.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in scannerWaitBlock3.components:
    ...
    thisExp.currentRoutine = startTaskBlock3
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        if text_26.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_26.tStartRefresh + .5-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # has the Routine been forcibly ended?
        if startTaskBlock3.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in startTaskBlock3.components:
    ...
    for thisDecisionLoop in decisionLoop:
        ...
        if decision_block:
            ...
            # === LOG FIRST STIMULUS ONSET TIMESTAMP ===
            scanner_clock = globals().get('scanner_clock', None)
            if scanner_clock is not None:
                stimulus1_onset_time = scanner_clock.getTime()
            else:
                stimulus1_onset_time = None
                print("WARNING: scanner_clock not set")
        
            globals()['stimulus1_onset_time'] = stimulus1_onset_time
            globals()['stimulus1_side'] = first_side
            globals()['stimulus1_fruit'] = first_fruit
            print(f"Stimulus 1 onset: {stimulus1_onset_time:.4f}s since scanner sync" if stimulus1_onset_time else "Stimulus 1 onset: no clock ref")
        ...
        decisionStimulus1.maxDuration = 2
        # keep track of which components have finished
        ...
        thisExp.currentRoutine = decisionStimulus1
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # update/draw components on each frame
            # is it time to end the Routine? (based on local clock)
            if tThisFlip > decisionStimulus1.maxDuration-frameTolerance:
                decisionStimulus1.maxDurationReached = True
                continueRoutine = False
            ...
            # has the Routine been forcibly ended?
            if decisionStimulus1.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionStimulus1.components:
        ...
        # Random duration (configurable): default 1/2/3 seconds
        iti1_values = globals().get('DECISION_ITI12_VALUES', [1, 2, 3])
        iti1_duration = random.choice(iti1_values)
        print(f"ITI 1: {iti1_duration} seconds")
        
        # Store in globals for use in Builder duration and logging
        globals()['iti1_duration'] = iti1_duration
        ...
        thisExp.currentRoutine = decisionITI1
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if decisionITI1.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionITI1.components:
        ...
        if decision_block:
            ...
            # === LOG SECOND STIMULUS ONSET TIMESTAMP ===
            scanner_clock = globals().get('scanner_clock', None)
            if scanner_clock is not None:
                stimulus2_onset_time = scanner_clock.getTime()
            else:
                stimulus2_onset_time = None
                print("WARNING: scanner_clock not set")
        
            globals()['stimulus2_onset_time'] = stimulus2_onset_time
            globals()['stimulus2_side'] = second_side
            globals()['stimulus2_fruit'] = second_fruit
            print(f"Stimulus 2 onset: {stimulus2_onset_time:.4f}s since scanner sync" if stimulus2_onset_time else "Stimulus 2 onset: no clock ref")
        ...
        decisionStimulus2.maxDuration = 2
        # keep track of which components have finished
        ...
        thisExp.currentRoutine = decisionStimulus2
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # update/draw components on each frame
            # is it time to end the Routine? (based on local clock)
            if tThisFlip > decisionStimulus2.maxDuration-frameTolerance:
                decisionStimulus2.maxDurationReached = True
                continueRoutine = False
            ...
            # has the Routine been forcibly ended?
            if decisionStimulus2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionStimulus2.components:
        ...
        # Random duration (configurable): default 1/2/3 seconds
        iti2_values = globals().get('DECISION_ITI12_VALUES', [1, 2, 3])
        iti2_duration = random.choice(iti2_values)
        print(f"ITI 2: {iti2_duration} seconds")
        
        # Store in globals for use in Builder duration and logging
        globals()['iti2_duration'] = iti2_duration
        ...
        thisExp.currentRoutine = decisionITI2
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if decisionITI2.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionITI2.components:
        ...
        if decision_block:
            ...
            globals()['decision_start_time'] = decision_start_time
            globals()['decision_timed_out'] = False
            # Reset choice timestamp for this trial
            ...
            # === LOG BOTH-FRUITS ONSET TIMESTAMP ===
            scanner_clock = globals().get('scanner_clock', None)
            if scanner_clock is not None:
                both_fruits_onset_time = scanner_clock.getTime()
            else:
                both_fruits_onset_time = None
                print("WARNING: scanner_clock not set")
            globals()['both_fruits_onset_time'] = both_fruits_onset_time
            print(f"Both fruits onset: {both_fruits_onset_time:.4f}s since scanner sync" if both_fruits_onset_time else "Both fruits onset: no clock ref")
        ...
        thisExp.currentRoutine = decisionMaking
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # Get the response limit (max seconds to respond) - NOT the timeout screen duration
            # decision_timeout_duration in globals is for the TIMEOUT SCREEN (1.5s or 0) - set only in EndRoutine
                ...
                if 'decisionMaking' in locals():
                    if hasattr(decisionMaking, 'duration'):
                        ...
                        if routine_duration > 0:
                            decision_response_limit = routine_duration
            ...
            # Check for timeout (use response limit, e.g. 2.0 seconds)
            try:
                elapsed_time = time.time() - decision_start_time
                
                if elapsed_time >= decision_response_limit:
                    # Timeout - no response (routine will end at the response limit)
                    if not globals().get('choice', ""):
                        choice = ""
                        decision_rt = decision_response_limit
                        decision_timed_out = True
                        # Store in globals for EndRoutine and decisionTimeout routine
                        globals()['decision_timed_out'] = True
                        globals()['decision_rt'] = decision_response_limit
                        globals()['decision_choice_time'] = None  # No choice made
                        # Set timeout screen duration HERE so decisionTimeout sees it even if EndRoutine ordering differs
                        globals()['decision_timeout_duration'] = globals().get('TIMEOUT_SCREEN_DURATION', 1.5)
                        print(f"Decision {decision_index+1}: No response within {decision_response_limit:.1f} seconds")
                    continueRoutine = False
            except:
                decision_timed_out = False
                    ...
                    try:
                        ...
                        for key in keys:
                            ...
                            if key == 'num_1' or key == '1':
                                ...
                                decision_rt = time.time() - decision_start_time
                                decision_timed_out = False
                                # === LOG CHOICE TIMESTAMP ===
                                scanner_clock = globals().get('scanner_clock', None)
                                decision_choice_time = scanner_clock.getTime() if scanner_clock else None
                                # Store in globals
                                globals()['choice'] = choice
                                globals()['decision_rt'] = decision_rt
                                globals()['decision_timed_out'] = False
                                globals()['decision_choice_time'] = decision_choice_time
                                print(f"Decision {decision_index+1}: Chose left (key 1, RT={decision_rt:.2f}s, scanner_time={decision_choice_time:.4f}s)" if decision_choice_time else f"Decision {decision_index+1}: Chose left (key 1, RT={decision_rt:.2f}s)")
                                continueRoutine = False
                                # Clear events after processing
                                try:
                                    event.clearEvents()
                                ...
                                globals()['decision_timed_out'] = False
                                globals()['decision_choice_time'] = decision_choice_time
                                print(f"Decision {decision_index+1}: Chose right (key 3, RT={decision_rt:.2f}s, scanner_time={decision_choice_time:.4f}s)" if decision_choice_time else f"Decision {decision_index+1}: Chose right (key 3, RT={decision_rt:.2f}s)")
                                continueRoutine = False
                                # Clear events after processing
                                try:
                                    event.clearEvents()
            ...
            # has the Routine been forcibly ended?
            if decisionMaking.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionMaking.components:
        ...
        # Get choice, decision_rt, and decision_timed_out from globals (set in EachFrame)
        choice = globals().get('choice', "")
        # Response limit (seconds): comes from EachFrame (preferred), otherwise global default
        decision_response_limit = globals().get('DECISION_TIMEOUT_DURATION', 2.0)
        decision_rt = globals().get('decision_rt', None)
        decision_timed_out = globals().get('decision_timed_out', False)
        
        # FALLBACK: If no choice and no RT, treat as timeout (EachFrame may not have run if Builder ended routine first)
        if not choice and decision_rt is None:
            decision_timed_out = True
        ...
        # If timed out, RT should equal the max response limit
        ...
        # Store for decisionTimeout routine - Builder uses $decision_timeout_duration for routine duration
        globals()['decision_timeout_duration'] = decision_timeout_screen_duration
        print(f"DEBUG decisionMakingEndRoutine: decision_timed_out={decision_timed_out}, decision_timeout_duration={decision_timeout_screen_duration}")
        ...
        thisExp.addData("stimulus1_onset_time", globals().get('stimulus1_onset_time', None))
        thisExp.addData("stimulus2_onset_time", globals().get('stimulus2_onset_time', None))
        thisExp.addData("choice_time", globals().get('decision_choice_time', None))
        ...
        # Per stimulus diagram:
        # - Response < 2s: crosshair = (2 - RT) + ITI (remaining response window + base ITI)
        # - No response: Time out screen, then crosshair = full ITI (2/3/4 s)
        import random
        timeout_screen_duration = globals().get('TIMEOUT_SCREEN_DURATION', 1.5)
        decision_response_limit = globals().get('DECISION_TIMEOUT_DURATION', 2.0)
        ...
        if decision_timed_out:
            # Full ITI after timeout screen (diagram: No response → Time out → I.T.I.)
            iti3_duration = float(iti3_total)
            iti3_remaining_response = 0.0  # No remaining response window
            print(f"ITI 3 (timeout): crosshair={iti3_duration:.2f}s (full ITI after timeout)")
        else:
            # Crosshair = remaining response time + ITI (diagram: Response → [2-RT] + [I.T.I.])
            ...
            print(f"ITI 3 (responded): RT={decision_rt_value:.3f}s, (2-RT)={iti3_remaining_response:.3f}s + ITI_base={iti3_total}s = crosshair={iti3_duration:.3f}s")
        
        # Ensure iti3_duration is in globals so Builder can access it
        globals()['iti3_duration'] = iti3_duration
        
        # Log ITI durations (for verification: iti3 = (2-RT) + base when responded)
        ...
        thisExp.addData("iti3_remaining_response", iti3_remaining_response)  # (2-RT) for responded trials
        thisExp.addData("iti3_base", iti3_total)  # Base ITI (2/3/4 s) chosen for this trial
        ...
        try:
            ...
            if data_logger:
                ...
                data_logger.log_decision_trial(
                    ...
                    stimulus1_onset_time=globals().get('stimulus1_onset_time', None),
                    stimulus2_onset_time=globals().get('stimulus2_onset_time', None),
                    both_fruits_onset_time=globals().get('both_fruits_onset_time', None),
                    choice_time=globals().get('decision_choice_time', None),
                )
        except Exception as e:
        ...
        # Safeguard: if we timed out but duration is 0 (race condition), use full timeout
        if decision_timed_out and decision_timeout_duration <= 0:
            decision_timeout_duration = globals().get('TIMEOUT_SCREEN_DURATION', 1.5)
            globals()['decision_timeout_duration'] = decision_timeout_duration
            print("DEBUG decisionTimeout: decision_timed_out=True but duration=0, forcing duration=1.5")
        ...
        thisExp.currentRoutine = decisionTimeout
        ...
        while continueRoutine:
            ...
            # Duration controlled by decisionTimeout.maxDuration (set in Begin Routine from decision_timeout_duration)
            # When decision_timeout_duration is 0, end immediately; otherwise Builder's maxDuration handles timing
            
            decision_timeout_duration = globals().get('decision_timeout_duration', 0)
            if decision_timeout_duration <= 0:
                # No timeout - end routine immediately (skip screen)
                continueRoutine = False
            ...
            # has the Routine been forcibly ended?
            if decisionTimeout.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionTimeout.components:
        ...
        # ITI 3 - Crosshair screen (duration: (2-RT)+ITI when responded, full ITI when timed out)
        # Duration is set in decisionMaking End Routine as iti3_duration
        # NOTE: iti3_duration is in globals() - must read explicitly for use in Each Frame
        iti3_duration = globals().get('iti3_duration', 2.0)
        decision_timed_out = globals().get('decision_timed_out', False)
        # Log what we're actually showing (Builder uses $iti3_duration for routine stopVal)
        print(f"ITI3 Begin: showing crosshair for iti3_duration={iti3_duration:.3f}s (timed_out={decision_timed_out})")
        ...
        print(f"Showing ITI 3 crosshair (duration: {iti3_duration}s)")
        ...
        thisExp.currentRoutine = decisionITI3
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if decisionITI3.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in decisionITI3.components:
